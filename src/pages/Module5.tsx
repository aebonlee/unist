...
const module5Data = {
  id: 'module5-kamp',
  icon: 'fa-flask',
  title: '모듈 5 · KAMP 화학공정 품질 데이터 분석',
  titleEn: 'Module 5 · KAMP Chemical Process Quality Analysis',
  sections: [
    {
      title: "왜 '화학 공정' 사례인가",
      titleEn: "Why a 'Chemical Process' Case",
      content: `이 모듈은 이 과정의 **핵심 실습**입니다. 앞선 모듈에서 익힌 데이터 문해력·클라우드 분석 환경·프롬프트 작성법을 한데 모아, **진짜 화학 공정 데이터로 1회 완결형 분석**을 끝까지 수행합니다.
...
이 1시간이 끝나면, 여러분은 화학 공정 데이터 한 벌을 받아 **전처리 → 탐색 → 예측 → 최적조건 도출 → 리포트**까지 스스로 끌고 갈 수 있게 됩니다.`,
      contentEn: `This is the **core hands-on module** of the course. It brings together the data literacy, cloud analysis environment, and prompting skills from earlier modules to run **one complete, self-contained analysis on real chemical process data.**
...
겉으로 보기에도 B-002, B-004(저온·저pH·짧은 체류)는 불량, B-001, B-003(고온·중성·긴 체류)은 양품입니다. **"운"이 아니라 운전조건의 차이**일 가능성이 보입니다. 이 직관을 데이터로 검증하는 것이 이 모듈의 일입니다.`,
      contentEn: `We drive the analysis through **one field story** (a learning scenario modeled on KAMP data).
...
The data is a **batch log**: each **row is one batch**, each **column a variable**, forming an **input → output** structure (operating conditions → yield/verdict). This is a **supervised learning** problem — **classification** (good/defective) plus **regression** (predicting the yield value). Even at a glance, low-temperature, low-pH, short-residence batches are defective while warmer, neutral, longer-residence batches are good — suggesting it is **not luck but operating conditions.** Verifying that intuition with data is this module's job.`,
    },
    {
      title: 'STEP 1 · 공정 변수 명세',
      titleEn: 'STEP 1 · Process Variable Specification',
      content: `분석에 앞서 **변수가 무엇을 의미하는지** 정확히 알아야 합니다. 화학 공정에서는 변수 하나하나가 품질에 직결되므로, 명세를 읽는 것이 곧 도메인 지식입니다.
...
같은 데이터라도 변수의 화학적 의미를 알면 해석이 깊어집니다. 예를 들어 "체류시간이 짧으면 수율이 낮다"는 결과를, 화학을 아는 사람은 **"반응이 완결되기 전에 배출돼 미반응 원료가 남았다"**고 해석합니다. **도메인 지식 + 데이터 = 가장 강한 분석가**입니다.`,
      contentEn: `Before analysis you must know **what each variable means.** In a chemical process every variable connects directly to quality, so reading the spec *is* domain knowledge.
...
**Domain knowledge enriches analysis**: a chemist reads "short residence time → low yield" as "the reaction was discharged before completion, leaving unreacted feed." Domain knowledge + data = the strongest analyst.`,
    },
    {
      title: 'STEP 2 · 전처리 — 분석 가능한 표 만들기',
      titleEn: 'STEP 2 · Preprocessing — Making an Analyzable Table',
      content: `현실의 공정 데이터는 결코 깨끗하지 않습니다. 센서가 가끔 값을 빠뜨리고, 오작동으로 비현실적 값을 찍기도 합니다. **전처리**는 이 지저분한 데이터를 분석 가능한 표로 정리하는, 가장 손이 많이 가는 단계입니다.
...
> "Garbage in, garbage out." — 더러운 데이터를 넣으면 더러운 결과가 나옵니다. 화려한 모델보다 **깨끗한 데이터**가 좋은 분석의 8할입니다.`,
      contentEn: `Real process data is never clean — sensors drop values and occasionally log impossible ones. **Preprocessing** turns this mess into an analyzable table and is the most labor-intensive step.
...
**Do not blindly trust** AI-generated code — inspect the resulting table: are flagged rows truly abnormal, are labels correct? As the saying goes, "garbage in, garbage out" — **clean data**, not a fancy model, is 80% of good analysis.`,
    },
    {
      title: 'STEP 3 · 탐색 분석 (기초통계 + 상관)',
      titleEn: 'STEP 3 · Exploratory Analysis (Statistics + Correlation)',
...
> **상관관계 ≠ 인과관계.** 온도와 수율이 함께 움직인다고 해서 "온도가 수율의 원인"이라고 단정하면 안 됩니다. 숨은 제3의 변수가 둘 다를 움직였을 수도 있습니다. 상관은 **"어디를 더 파볼지"를 알려주는 출발점**이지, 결론이 아닙니다. 최종 판단은 도메인 지식과 검증으로 보완합니다.`,
      contentEn: `With clean data in hand, we **listen to what it says.** Before modeling, **exploratory data analysis (EDA)** uses basic statistics and correlation to grasp the data's character.
...
Remember one sentence: **correlation ≠ causation.** That temperature and yield move together does not prove temperature *causes* yield — a hidden third variable could drive both. Correlation tells you **where to dig next**, not the conclusion; final judgment needs domain knowledge and verification.`,
    },
    {
      title: 'STEP 4 · 불량 배치 예측 모델',
      titleEn: 'STEP 4 · Defective-Batch Prediction Model',
      content: `이제 핵심 질문 2번에 답합니다 — **"불량 배치를 미리 예측할 수 있나?"** 운전조건(입력)으로 양품/불량(출력)을 맞히는 **예측 모델**을 만듭니다. 코드는 생성형 AI가 쓰고, 우리는 절차와 평가를 이해합니다.
...
모델이 꼽은 중요 변수가 STEP 3 상관분석과 일치하면(온도·체류시간 상위) 결과의 신뢰도가 높아집니다. 이 변수들이 곧 **현장에서 집중 관리할 포인트**입니다.`,
      contentEn: `Now we answer the second question — **"can defective batches be predicted in advance?"** We build a **prediction model** that maps operating conditions (input) to good/defective (output). AI writes the code; we understand the procedure and evaluation.
...
**The accuracy trap**: if defects are rare (say 5%), a model that always says "all good" still scores 95% accuracy — yet catches **zero** defects and is useless. So never look at accuracy alone. In chemical quality work, what matters is **not missing defects**, i.e., **recall** and the **confusion matrix**. Missing a defect (calling a defective batch good) is the most dangerous error, leading to claims, recalls, and safety issues — so we focus on recall. When the model's important variables match STEP 3's correlations (temperature, residence time on top), confidence rises, and those become the **field control points.**`,
    ...
    {
      title: 'STEP 5 · 최적 운전조건 & 산출물',
      titleEn: 'STEP 5 · Optimal Operating Conditions & Deliverables',
      content: `마지막 핵심 질문 — **"수율이 가장 높은 운전조건 구간은?"** 분석 결과를 모아 **데이터가 가리키는 최적 운전 구간**을 도출하고, 현장이 바로 쓸 수 있는 산출물로 만듭니다.
...
> 분석은 **차트로 끝나면 안 됩니다.** "그래서 무엇을 할 것인가"라는 **행동(인사이트)**으로 이어질 때 비로소 가치가 생깁니다. 산출물은 그 행동을 현장에 전달하는 통로입니다.`,
      contentEn: `The final question — **"where is the highest-yield operating window?"** We gather the results into the **optimal window the data points to** and turn it into deliverables the floor can use immediately.
...
- **NotebookLM**: 명세서·리포트를 올려 지식베이스로 구조화`,
      contentEn: `Chemical data has **special characteristics** beyond ordinary data, so handling it **responsibly** matters as much as analytical skill.
...
**Next — Module 6, Visualizing Analysis Results**: insights not shown well are not communicated, so we turn these results into control charts, trend graphs, and dashboards that management and the floor grasp at a glance. Resources: KAMP (kamp-ai.kr/aidataList), Colab + Gemini, NotebookLM.`,
    },
  ],
};
    ...
    <>
      <section className="page-header-ed">
        <div className="container">
          ...
          <h2>{isKo ? 'KAMP 화학공정 품질 데이터 분석' : 'KAMP Chemical Process Quality Analysis'}</h2>
          <p>{isKo ? '한빛케미칼 반응공정 — 수율 예측과 최적 운전조건 도출 · 1.0H' : 'Reaction process yield prediction and optimization · 1.0H'}</p>
        </div>
      </section>
      <GuidePage
        seoTitle="모듈 5 · KAMP 화학공정 품질 데이터 분석"
        seoTitleEn="Module 5 · KAMP Chemical Process Quality Analysis"
        seoDescription="KAMP 화학 공정 데이터로 반응공정 운전변수와 수율의 관계를 분석하고, 불량 배치를 예측하며 최적 운전조건을 도출하는 화학 트랙 핵심 실습 강의안"
        path="/module5"
        dataFiles={[module5Data]}
      />
    </>
  );
}
