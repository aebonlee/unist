...
const module6Data = {
  id: 'module6-viz',
  icon: 'fa-chart-line',
  title: '모듈 6 · AI 도구 활용 분석 결과 시각화',
  titleEn: 'Module 6 · Visualizing Results with AI Tools',
  sections: [
    {
      title: 'NotebookLM으로 지식 구조화 [프로젝트]',
      titleEn: 'Structuring Knowledge with NotebookLM [Project]',
      content: `이번 모듈은 강의가 아니라 **마무리 프로젝트**입니다. 앞선 다섯 모듈에서 만든 분석 결과(코드·표·차트·인사이트)를 **하나의 작품**으로 묶어냅니다. 첫 단계는 **NotebookLM**으로 흩어진 자료를 "검색 가능한 지식"으로 구조화하는 것입니다.
...
> **프로젝트 미션 1**: 다섯 모듈의 산출물을 NotebookLM에 업로드하고, ①핵심 요약 Q&A 3개 ②마인드맵 1장 ③오디오 개요 1개를 생성해 보세요. 이것이 우리 분석의 "지식 베이스"가 됩니다.`,
      contentEn: `This module is not a lecture but a **final project**: bundling the analysis results from the previous five modules (code, tables, charts, insights) into a single deliverable. The first step is using **NotebookLM** to structure scattered materials into "searchable knowledge."
...
> **프로젝트 미션 2**: 5개 섹션 구조에 맞춰 Canvas로 최종 리포트 초안을 생성하고, 현장 맥락 2~3개를 직접 추가해 완성하세요.`,
      contentEn: `With knowledge structured in NotebookLM, **Gemini Canvas** integrates all analysis into **one complete report**. Canvas is a workspace for co-editing documents — you generate a draft, select parts, and rewrite to refine.
...
Canvas can also convert a finished report into an **interactive app (web preview)** with one click, turning a static report into a clickable tool — leading into the dashboard in the next section. Mission: generate the five-section draft and finish it with two or three pieces of field context.`,
    },
    {
      title: '앱 대시보드 기획 & 제작 [프로젝트]',
      titleEn: 'Planning & Building an App Dashboard [Project]',
      content: `리포트가 "지난 일을 설명하는 문서"라면, 대시보드는 **"지금을 보여주는 운영 도구"**입니다. 분석을 1회성 보고에서 **매일 보는 화면**으로 전환하는 단계입니다.
...
> **프로젝트 미션 3**: 내 업무의 KPI 3~6개를 정하고, 레이아웃을 스케치한 뒤, AI로 대시보드 시안을 생성하세요.`,
      contentEn: `If a report "explains what happened," a dashboard is an **operational tool that shows what is happening now** — turning analysis from a one-off report into a **screen you check daily**.
...
- [x] 결과를 **리포트·발표 자료·대시보드**로 전달한다`,
      contentEn: `Completing the project leaves **four tangible deliverables** — the fruit of the six-hour course.
...
By the end you can: turn a vague field problem into a data-defined question, clean and analyze data with Colab + Gemini without coding, use PTCF prompts for accurate analysis, find patterns and outliers with statistics and visualization, analyze text data like reviews, build a simple quality-prediction model, and deliver results as reports, presentations, and dashboards.`,
    ...
    {
      title: '현업 적용 가이드 & 다음 단계',
      titleEn: 'Applying It at Work & Next Steps',
      content: `과정이 끝나면 가장 흔한 질문은 "그래서 내 업무에 어떻게 적용하나"입니다. 거창하게 시작하지 마세요. **작게 시작해 한 번 성공**하는 것이 핵심입니다.
...
Right now, steps 1 and 2 are the most practical; prioritize step 3 if your field is sensor-time-series heavy, or step 4 if you report frequently. The path: now → pandas/ML basics → domain specialization (time series/BI) → automation and agents.`,
    },
    {
      title: '모듈 6 정리 & 체크리스트',
      titleEn: 'Module 6 Summary & Checklist',
      content: `### 핵심 요약
...
> 이제 데이터는 '읽는 것'이 아니라 '함께 만드는 것'입니다. 여러분은 현장 지식에 데이터 문해력과 AI 활용력을 더한, 가장 강력한 분석가가 되었습니다.`,
      contentEn: `### Summary
...
Data is no longer something you "read" but something you "build together." With domain knowledge plus data literacy and AI skills, you have become the strongest analyst.`,
    },
  ],
};
    ...
    <>
      <section className="page-header-ed">
        <div className="container">
          ...
          <h2>{isKo ? 'AI 도구 활용 분석 결과 시각화' : 'Visualizing Results with AI Tools'}</h2>
          <p>{isKo ? 'NotebookLM 지식화 · Canvas 리포트 · 앱 대시보드 · 1.0H' : 'NotebookLM, Canvas report, app dashboard · 1.0H'}</p>
        </div>
      </section>
      <GuidePage
        seoTitle="모듈 6 · AI 도구 활용 분석 결과 시각화"
        seoTitleEn="Module 6 · Visualizing Results with AI Tools"
        seoDescription="NotebookLM으로 분석 결과를 지식 구조화하고, Gemini Canvas로 최종 분석 리포트와 앱 대시보드를 제작하는 프로젝트 강의안"
        path="/module6"
        dataFiles={[module6Data]}
      />
    </>
  );
}
