import{u as i,j as e}from"./index-DL44mQ5e.js";import{G as n}from"./GuidePage-DbckXl-t.js";import"./SEOHead-BeUijBbz.js";const o={id:"module4-voc",icon:"fa-comments",title:"모듈 4 · 고객 반응 데이터 수집 및 분석",titleEn:"Module 4 · Voice of Customer Collection and Analysis",sections:[{title:"왜 고객 반응(VOC) 데이터인가",titleEn:"Why Voice of Customer Data",content:`이번 모듈은 **1시간 실습** 모듈입니다. 공정·센서 데이터를 넘어, 이번에는 **사람이 직접 남긴 데이터** — 즉 **고객의 목소리(VOC, Voice of Customer)**를 다룹니다. 앱스토어 리뷰를 자동으로 수집하고, 생성형 AI로 테마를 분류하고, 제품 개선 리포트까지 한 번에 완성하는 것이 목표입니다.
...
모듈 1에서 배운 분석 워크플로우 4단계(수집 → 전처리 → 분석 → 인사이트)를 **VOC 데이터에 그대로 적용**하는 실습입니다. 도구는 Google Colab + Gemini(수집·분석)와 Gemini Canvas(리포트)를 사용합니다.`,contentEn:`This is a **1-hour hands-on** module. Beyond process and sensor data, we now work with data written by people directly — the **Voice of Customer (VOC)**. The goal is to automatically collect app-store reviews, classify themes with generative AI, and produce a product-improvement report in one flow.
...
This practice applies the 4-step workflow from Module 1 (Collect → Preprocess → Analyze → Insight) directly to VOC data, with the flow \`review crawling → sentiment/theme classification → pain-point extraction → improvement report (Canvas)\`, using Google Colab + Gemini and Gemini Canvas.`},{title:"앱스토어 리뷰 자동 수집 [실습]",titleEn:"Automatic App Review Collection [Hands-on]",content:`리뷰를 한 건씩 복사·붙여넣기 하는 것은 불가능합니다. **크롤링(crawling)**으로 자동 수집합니다. 수집 프로세스는 4단계입니다.
...
**Ethics and security**: collect **public reviews only**, avoid excessive requests in a short time (to prevent server load and blocking), and **exclude personally identifiable information** such as nicknames from storage and analysis — score, date, and body are enough. Check the service's Terms of Service and robots policy, and confirm permissions before any commercial redistribution.`},{title:"테마 분석 & 페인포인트 추출 [실습]",titleEn:"Theme Analysis & Pain-point Extraction [Hands-on]",content:`리뷰 500개를 그냥 읽으면 시간만 가고 결론이 안 납니다. 생성형 AI로 **리뷰를 인사이트로 바꾸는 3단계**를 거칩니다.
...
> Canvas가 만든 초안은 **출발점**이지 정답이 아닙니다. 환각(없는 수치 생성)이 없는지, 우리 현장 상황과 맞는지 **사람이 반드시 검증**하고 다듬습니다. 좋은 리포트는 "AI가 쓴 글"이 아니라 "AI로 빠르게 초안 잡고 전문가가 마무리한 글"입니다.`,contentEn:`Analysis alone is only half the job — insight must become a **report that readers (management, planning, development) can act on immediately**. **Gemini Canvas** auto-generates the document, tables, and charts.
...
The Canvas draft is a **starting point, not the answer**. A human must verify there are no hallucinated numbers and that it fits the real situation. A good report is not "written by AI" but "drafted quickly with AI and finished by an expert."`},{title:"화학·제조 중소기업에 적용하기",titleEn:"Applying to Chemical & Manufacturing SMEs",content:`이번 실습은 앱 리뷰를 예로 들었지만, **같은 흐름은 우리 회사 데이터에 그대로** 적용됩니다. VOC는 앱에만 있는 것이 아닙니다.
...
> **VOC를 품질·영업에 연결하기**: 고객의 목소리는 단순한 불평이 아니라 **무료 컨설팅**입니다. 반복되는 클레임은 다음 제품 개선의 1순위 후보이고, 칭찬받은 기능은 영업의 강점 메시지가 됩니다. 화학 중소기업도 흩어진 고객 피드백을 데이터로 모으면, **감이 아닌 근거로 제품과 영업을 개선**할 수 있습니다.`,contentEn:`This practice used app reviews as the example, but the **same flow applies directly to your own company's data**. VOC is not limited to apps.
...
The voice of the customer is not mere complaint but **free consulting**: recurring claims are the top candidates for the next product improvement, and praised features become sales strengths. Gathering scattered feedback as data lets even a small chemical company improve products and sales **with evidence, not gut feeling**.`},{title:"모듈 4 정리 & 체크리스트",titleEn:"Module 4 Summary & Checklist",content:`### 핵심 요약
...
다음 모듈에서는 고객 데이터에서 **공정 데이터**로 돌아갑니다. 스마트제조 빅데이터 플랫폼 **KAMP**의 화학공정 품질 데이터를 활용해, 실제 제조 현장의 품질 변수를 분석하고 불량을 좌우하는 핵심 인자를 찾아내는 실습을 진행합니다.`,contentEn:`### Summary
...
The next module returns from customer data to **process data**, using the KAMP smart-manufacturing platform's chemical-process quality data to analyze real production-floor quality variables and identify the key factors driving defects.`}]};function l(){const{language:a}=i(),t=a==="ko";return e.jsxs(e.Fragment,{children:[e.jsx("section",{className:"page-header-ed",children:e.jsxs("div",{className:"container",children:[e.jsx("div",{className:"eyebrow",children:"Module 04"}),e.jsx("h2",{children:t?"고객 반응 데이터 수집 및 분석":"Voice of Customer Collection and Analysis"}),e.jsx("p",{children:t?"앱스토어 리뷰 크롤링 · 테마 분석 · 개선 리포트 · 1.0H":"App review crawling, theme analysis, report · 1.0H"})]})}),e.jsx(n,{seoTitle:"모듈 4 · 고객 반응 데이터 수집 및 분석",seoTitleEn:"Module 4 · Voice of Customer Collection and Analysis",seoDescription:"앱스토어 리뷰 데이터 자동 수집(크롤링), 테마 분석과 핵심 페인포인트 추출, Gemini Canvas 기반 제품 개선 리포트 작성을 다루는 실습 강의안",path:"/module4",dataFiles:[o]})]})}export{l as default};
